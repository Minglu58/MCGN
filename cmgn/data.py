...
from cmgn.modules import AudioCLIP
...
def collate_fn(batch):
    ...
    #         if key not in ['text', 'video', 'audio']:
    #             print(f"Key: {key}")  # 打印每个字段的维度
    #             print(f"Shape: {value}")
    #         if key in ['text', 'video', 'audio']:
    #             print(f"Key: {key}")  # 打印每个字段的维度
...
class VideoData(pl.LightningDataModule):
    ...
    def _dataset(self, train):
        ...
        elif hasattr(self.args, 'avs') and self.args.avs:
            Dataset = AVSDataset
            dataset = Dataset(self.args.data_path, self.args.sequence_length, self.args.text_emb_model,
                              self.args.audio_emb_model, train=train, resolution=self.args.resolution,
                              load_vid_len=self.args.load_vid_len, image_channels=self.args.image_channels,
    ...
    def add_data_specific_args(parent_parser):
        ...
        parser.add_argument('--avs', action='store_true')
...
class TAVDataset(data.Dataset):
    ...
    def __init__(self, data_folder, sequence_length, text_emb_model, audio_emb_model, 
        ...
        self.text_paths = [path.replace("/mp4/", "/txt/").replace(".mp4", ".txt") for path in self.video_paths]
        ...
        if text_emb_model == 'bert':
            ...
            self.tokenizer = CLIPTokenizer.from_pretrained('cmgn/modules/cache/clip-vit-large-patch14')
            self.transformer = CLIPTextModel.from_pretrained('cmgn/modules/cache/clip-vit-large-patch14')
    ...
    def __getitem__(self, idx):
        ...
        if self.audio_emb_model == 'STFT':
            ...
            # audio = audio[:144960] #TO DO: for audioset_drums&landscape
...
class AVSDataset(data.Dataset):
    ...
    def __init__(self, data_folder, sequence_length, text_emb_model, audio_emb_model, 
        ...
        self.video_paths = [path.replace("/txt_3s/", "/mp4_3s/").replace(".txt", ".mp4") for path in self.text_paths]
        self.audio_paths = [path.replace("/txt_3s/", "/wav_3s/").replace(".txt", ".wav") for path in self.text_paths]
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                # random_clip_idx = np.random.randint(self._clips.num_clips())
                # # # 获取该随机片段及对应的原始视频索引
                # video, _, _, video_idx = self._clips.get_clip(random_clip_idx)
        ...
        if self.audio_emb_model == 'STFT':
            ...
            stft = F.interpolate(stft, size=(96, 16), mode="bilinear", align_corners=False)
            return dict(**preprocess(video[start:end], self.resolution, sample_every_n_frames=self.sample_every_n_frames), 
                        text=outputs, raw_text=text, stft=stft, path=self.video_paths[video_idx], fps=fps)
        elif self.audio_emb_model == 'wav2clip' or self.audio_emb_model == 'audioclip' or self.audio_emb_model == "beats":
            audiofile = self.audio_paths[video_idx]
            sr = 48000
            audio, _ = librosa.load(audiofile, sr=sr)
