...
def main():
    ...
    for i in range(args.num_samples):
        ...
        if args.use_temporal_conv == True:
            c = th.concat((c_t, c_i, c_temp), dim=1)
        else:
            c = th.concat((c_t, c_i), dim=1)
        ...
        os.makedirs(os.path.join("./results", "stage1"), exist_ok=True)
        save_video_grid(sample_recon+0.5, os.path.join("./results", "stage1", f"video_%d.mp4"%(i)), 1)
